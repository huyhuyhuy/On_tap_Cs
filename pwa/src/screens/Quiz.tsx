          ...
          {current.options.map((option) => {
            ...
            return (
              <li key={option.key}>
                ...
                >
                  <span className="opt-key">{option.key}</span>
        ...
        {revealed ? (
          <div className={`feedback ${isCorrect ? 'ok' : 'bad'}`}>
            <p className="feedback-title">
              {isCorrect ? 'Đúng' : `Sai — đáp án đúng là ${current.answer}`}
            </p>
            {current.explanation ? (
          ...
          <p className="hint">Chọn một đáp án để kiểm tra.</p>
        )}
      </article>
    </div>
  )
